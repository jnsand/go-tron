...
import (
	...
	"github.com/jnsand/go-tron"

	"github.com/jnsand/go-tron/abi"
	"github.com/jnsand/go-tron/account"
	"github.com/jnsand/go-tron/address"
)
...
func (r GetaccountResource) FreeNetAvailable() int {
	return r.FreeNetLimit - r.FreeNetUsed
}

func (c *Client) GetAccountResource(addr string) (GetaccountResource, error) {

	add, err := address.FromBase58(addr)
	if err != nil {
		return GetaccountResource{}, err
	}
	...
	var acc GetaccountResource
	if err := c.post("wallet/getaccountresource", &request, &acc); err != nil {
		return GetaccountResource{}, err
	}
	return acc, nil
...
func (c *Client) TriggerSmartContract(acc account.Account, input CallContractInput) (*tron.Transaction, bool, error) {
	request := struct {
		ContractAddress  string `json:"contract_address"`
		FunctionSelector string `json:"function_selector"`
		Parameter        string `json:"parameter"`
	...
	if !input.Function.Payable() {
		if input.CallValue > 0 {
			return nil, false, errors.New("client: cannot send tron to non-payable function")
		}
	}
	...
	return response.Transaction, response.Result.Result, nil
